    ...
    for i in range(num_channels):
        if data[i, 0] != initial_values[i]:
            raise ValueError(
                ...
                + " is inconsistent with the initial value for channel".format(
                    channels[i]
                )
            )
...
def get_cpc(string):
    ...
    if variable is None or is_nan(variable):
        raise ValueError(
            "No CPC score available. Is your code trying to load labels from the hidden data?"
        )
    return variable
...
def load_challenge_labels(folder):
    ...
    for i in range(num_patients):
        patient_metadata_file = os.path.join(
            folder, patient_folders[i], patient_folders[i] + ".txt"
        )
        patient_metadata = load_text_file(patient_metadata_file)
...
def save_challenge_outputs(filename, patient_id, outcome, outcome_probability, cpc):
    ...
    outcome_probability_string = "Outcome probability: {:.3f}".format(
        float(outcome_probability)
    )
    cpc_string = "CPC: {:.3f}".format(
        int(float(cpc)) if is_integer(cpc) else float(cpc)
    )
    output_string = (
        patient_string
        + "\n"
...
def load_challenge_outputs(folder, patient_ids):
    ...
    for i in range(num_patients):
        ...
        patient_id, outcome, outcome_probability, cpc = load_challenge_output(
            output_file
        )
        outcomes[i] = outcome
